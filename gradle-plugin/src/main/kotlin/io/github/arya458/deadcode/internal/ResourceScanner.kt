    ...
    fun scan(): ResourceModel {
        ...
        if (extension.scanConfigFiles) {
            extension.configDirs.forEach { configDir ->
                scanSpringConfigs(project.projectDir.resolve(configDir), referencedClassesFromSpring)
            }
        }
    ...
    private fun scanResources(
        ...
        resRoot.walkTopDown().filter { it.extension == "xml" }.forEach { file ->
            ...
            Regex("android:onClick=\"(\\w+)\"").findAll(xml).forEach {
                referencedMethods.add(MethodRef("", it.groupValues[1], "()V"))
            }
        }
    }
                ...
                try {
                    ...
                    cr.accept(object : ClassVisitor(Opcodes.ASM9) {
                        override fun visitField(
                            ...
                            if (descriptor == "I") declared.add(type to name)
                ...
                file.name.endsWith(".xml") -> {
                    try {
                        ...
                        for (i in 0 until elements.length) {
                            ...
                            if (classAttr != null) {
                                referencedClasses.add(classAttr.textContent.replace('.', '/'))
                            }
                        }
                    } catch (_: Exception) {
                    }
